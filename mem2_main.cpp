...
opt_analyze (opt_t * o, int argc, char * argv[])
{
  ...
  memset (&opt0, 0, sizeof(mem_opt_t));
	...
	while ((c = getopt(argc, argv, "g:xFi:o:n:1:2:3:b:t:paMCSPVYjk:c:v:s:r:A:B:O:E:U:w:L:d:T:Q:D:m:I:N:W:G:h:y:K:X:f")) != -1)
	{
    if (c == 'i')
...
read_align_init (bio_fq_t * fq, opt_t * opt)
{
  ...
  int l_rd;
  int is_pe;
  int max_num_mismatch;
  int min_num_match[2];
  int low_qual;
  double max_num_low_qual[2];
  double max_num_Ns[2];

  is_pe = fq->flag & BIO_FASTQ_PE;

  if (!fq_has_phred33_quals2(fq->fq[0].s))
    err_mesg ("This tool only accpets reads with Phred33 quality system!");
  if ((fq->flag & BIO_FASTQ_PE) && !fq_has_phred33_quals2(fq->fq[1].s))
    err_mesg ("This tool only accpets reads with Phred33 quality system!");
  low_qual = opt->low_qual + 33;

	if (need_filt) {
	  max_num_low_qual[0] = opt->lq_rate * (double)fq->rd_len[0];
	  max_num_Ns[0] = opt->n_rate * (double)fq->rd_len[0];
		if (fq->flag & BIO_FASTQ_AD_STR)
	  	min_num_match[0] = (int) ceil (0.5 * (double)fq->ad_str[0].l);
	  max_num_mismatch = 2;
	  if (is_pe) {
	    max_num_low_qual[1] = opt->lq_rate * (double)fq->rd_len[1];
	    max_num_Ns[1] = opt->n_rate * (double)fq->rd_len[1];
			if (fq->flag & BIO_FASTQ_AD_STR)
	    	min_num_match[1] = (int) ceil (0.5 * (double)fq->ad_str[1].l);
	  }
	
	  xrd_filter_set (fastq_filter, fq->flag, low_qual,
...
usage (void)
{
  mem_opt_t * mem_opt;
	...
	fprintf (stderr, "\n");
	fprintf (stderr, "Usage:  fast_aligner align [options] <reference> [fq1,fq2]\n");
	fprintf (stderr, "\n");
	...
	fprintf (stderr, "   -F   NUL   add the step of filtering reads with low quality\n");
	fprintf (stderr, "   -1   INT   low quality threshold [5], '-F' must be set\n");
	fprintf (stderr, "   -2   FLT   low quality rate [0.5], '-F' must be set\n");
	fprintf (stderr, "   -3   FLT   N rate threshold [0.05], '-F' must be set\n");
	fprintf (stderr, "   -f   NUL   re-run and overwrite all intermediate files\n");
	fprintf (stderr, "   -g   NUL   calculate the cigar of mate read and add it to the record of read\n");
	fprintf (stderr, "\n");
  ...
  free (mem_opt);
	...
	fprintf (stderr, "   1. fast_aligner align -i input.cfg -o out_dir hg19.fa\n");
	fprintf (stderr, "   2. fast_aligner align -b AAGTGACAA,AAGTGAGCCAAGGAGTTG -o out_dir hg19.fa input_r1.fq.gz,input_r2.fq.gz\n");
